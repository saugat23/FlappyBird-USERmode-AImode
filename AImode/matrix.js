export default class Matrix {
  ...
  static fromArray(arr) {
    // Create 1 column matrix from an array
  ...
  toArray() {
    // Flat the matrix into one dimensional array (going through columns first)
    const arr = [];
    for (let i = 0; i < this.rows; i += 1) {
      for (let j = 0; j < this.columns; j += 1) {
  ...
  add(n) {
    if (n instanceof Matrix) {
      // IF n is a matrix then do matrix sum
      for (let i = 0; i < this.rows; i += 1) {
        for (let j = 0; j < this.columns; j += 1) {
  ...
  static subtract(m1, m2) {
    // Static function for two matrices subtraction
  ...
  subtract(n) {
    if (n instanceof Matrix) {
      // IF n is a matrix then do matrix subtract
      for (let i = 0; i < this.rows; i += 1) {
        for (let j = 0; j < this.columns; j += 1) {
          this.matrix[i][j] -= n.matrix[i][j];
        }
  ...
  multiply(n) {
    // Regular function for scalar multiplication
    for (let i = 0; i < this.rows; i += 1) {
      for (let j = 0; j < this.columns; j += 1) {
        this.matrix[i][j] *= n;
      }
