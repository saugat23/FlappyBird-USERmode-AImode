  ...
  visualize(main, startX, startY, canvasWidth) {
    ...
    class Circle {
      ...
      display() {
        ...
        main.fill("black");
        main.strokeWeight(0);
        if (this.whichLayer === "input")
          main.text(Math.floor(this.label), this.posX, this.posY + 5);
    ...
    for (let i = 0; i < this.inputNodes; i += 1) {
      inputNodes.push(
        new Circle("input", this.input.toArray()[i], inputPosX, nodePosY)
      );
      nodePosY += 45;
    }
    nodePosY = startY;
    for (let i = 0; i < this.hiddenNodes; i += 1) {
      hiddenNodes.push(
        new Circle("hidden", this.hidden.toArray()[i], hiddenPosX, nodePosY)
      );
      nodePosY += 45;
    }
    nodePosY = startY;
    for (let i = 0; i < this.outputNodes; i += 1) {
      outputNodes.push(
        new Circle("output", this.output.toArray()[i], outputPosX, nodePosY)
      );
      nodePosY += 45;
    }
    ...
    for (let i = 0; i < this.weights_ih.rows; i += 1) {
      for (let j = 0; j < this.weights_ih.columns; j += 1) {
        ...
        main.strokeWeight(
          main.map(Math.abs(this.weights_ih.matrix[i][j]), 0, 1, 0.5, 4)
        );
        main.line(
          inputNodes[j].posX,
          inputNodes[j].posY,
    ...
    for (let i = 0; i < this.weights_ho.rows; i += 1) {
      for (let j = 0; j < this.weights_ho.columns; j += 1) {
        ...
        main.strokeWeight(
          main.map(Math.abs(this.weights_ho.matrix[i][j]), -1, 1, 0.5, 4)
        );
        main.line(
          hiddenNodes[j].posX,
          hiddenNodes[j].posY,
    ...
    for (let i = 0; i < this.outputNodes; i += 1) {
      if (this.output.toArray()[i] >= 0.5) {
        main.fill("white");
        main.ellipse(outputNodes[i].posX, outputNodes[i].posY, 40, 40);
      }
